...
def getInitDataset(S):
    ...
    for step in tqdm(S):            # loop over all one-step transitions
        ...
        if X.shape[0] == 0:
            X = np.array([step[0][0], step[0][1], step[0][2], step[0][3], step[1]])
        else:
            X = np.vstack((X, np.array([step[0][0], step[0][1], step[0][2], step[0][3], step[1]])))
...
def createInput(actionSet, X, traj):
    ...
    for action in tqdm(actionSet):
        ...
        for k in range(X.shape[0]):
            temp_arr.append([traj[k][-1][0], traj[k][-1][1], traj[k][-1][2], traj[k][-1][3], action])       # loop over all the states
...
def FQI_loop(envName, nbLoop = 3, nbIt = 180, nbTraj = 1000):
    ...
    for k in range(nbLoop):
        ...
        np.save(f"X_test_{k}.npy", XFull)
        np.save(f"y_test_{k}.npy", yFull)
        ...
        np.save(f"X_{k}.npy", XFull)
        np.save(f"y_{k}.npy", yFull)
            
        saveModel(est, f"model_evol_{k}")

    return est

def updateFQI(envName,alpha = 0.5, nbEpisode = 500, nbIt = 60, nbTraj = 500):
    """
    Function to put everything together for FQI
    """
